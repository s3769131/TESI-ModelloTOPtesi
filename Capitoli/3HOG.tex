...
\chapter{Istogrammi di gradienti orientati per estrazione di \emph{feature}} % Main chapter title
\label{cap:hog}
...
\begin{tikzpicture}[node distance = 2.5cm, every text node part/.style={align=center}]
    ...
    %\node [] (node2) at (4,0) {};
    \node [block_b] (combinazione) at (6,0) {$G=\sqrt{G_x^2+G_y^2}$ \\ $\theta=atan(\tfrac{G_y}{G_x})+\text{cost}$};
    \node[right of = combinazione, node distance=3.5cm](uscita){$\mathbf{G}(j,k)$};
...
\caption{Combinazione delle derivate per estrazione di contorni}
